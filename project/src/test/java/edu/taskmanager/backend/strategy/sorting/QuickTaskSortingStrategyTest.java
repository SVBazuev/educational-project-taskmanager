...
class QuickTaskSortingStrategyTest {
    ...
    void sort_emptyList_returnsEmpty() {
        List<Task> result = strategy.sort(new ArrayList<>());
    ...
    void sort_singleElement_returnsSameElement() {
        List<Task> result = strategy.sort(List.of(taskHigh));
    ...
    @Test
    @DisplayName("quickSort(): сортировка по убыванию приоритета через компаратор")
    void quickSort_withDescendingComparator_sortedCorrectly() {
        List<Task> input = List.of(taskLow, taskCritical, taskMedium, taskHigh);
    ...
    @Test
    @DisplayName("quickSort(): сортировка по возрастанию приоритета через компаратор")
    void quickSort_withAscendingComparator_sortedCorrectly() {
    ...
    @Test
    @DisplayName("quickSort(): сортировка по title через компаратор")
    void quickSort_withTitleComparator_sortedAlphabetically() {
    ...
    @Test
    @DisplayName("quickSort(): исходный список не изменяется после сортировки")
    void quickSort_doesNotMutateOriginalList() {
    ...
    @Test
    @DisplayName("quickSort(): возвращаемый список содержит те же объекты, что и исходный")
    ...
    @Test
    @DisplayName("quickSort(): большой список сортируется корректно")
    void quickSort_largeList_sortedCorrectly() {
        Priority[] priorities = Priority.values(); // LOW, MEDIUM, HIGH, CRITICAL
        List<Task> input = new ArrayList<>();
        for (int i = 0; i < 100; i++) {
            input.add(createTask((long) i, "Task " + i, priorities[i % priorities.length]));
        }
